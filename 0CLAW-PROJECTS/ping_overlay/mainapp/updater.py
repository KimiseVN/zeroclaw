...
def install_incremental_update(
    ...
    _APP_UNINSTALL_KEY_64 = (
        r"HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall"
        r"\{A1B2C3D4-E5F6-7890-ABCD-EF1234567890}_is1"
    )
    _APP_UNINSTALL_KEY_32 = (
        r"HKLM:\SOFTWARE\WOW6432Node\Microsoft\Windows\CurrentVersion\Uninstall"
        r"\{A1B2C3D4-E5F6-7890-ABCD-EF1234567890}_is1"
    )
    script_lines = [
        "param([int]$WaitPid,[string]$StageDir,[string]$InstallDir,[string]$ExePath,[string]$LogPath,[string]$StampPath,[string]$TargetVersion)",
        ...
        # Step 4: update Windows Installed Apps registry (DisplayVersion + DisplayName)
        "if ($TargetVersion -ne '') {",
        f"  foreach ($regPath in @('{_APP_UNINSTALL_KEY_64}', '{_APP_UNINSTALL_KEY_32}')) {{",
        "    if (Test-Path $regPath) {",
        "      try {",
        "        Set-ItemProperty -Path $regPath -Name 'DisplayVersion' -Value $TargetVersion -EA Stop",
        "        Set-ItemProperty -Path $regPath -Name 'DisplayName' -Value ('WWM Overlay v'+$TargetVersion) -EA Stop",
        "        Write-Log ('registry updated: DisplayVersion='+$TargetVersion+' at '+$regPath)",
        "      } catch {",
        "        Write-Log ('registry update failed at '+$regPath+': '+$_.Exception.Message)",
        "      }",
        "    }",
        "  }",
        "}",
