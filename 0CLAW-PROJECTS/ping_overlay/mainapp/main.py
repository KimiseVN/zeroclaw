...
# ── Early startup: purge stale loose .py files from _MEIPASS root ─────────────
# Old installer editions occasionally left application modules (e.g. net_utils.py,
# config.py) as physical .py files directly inside _internal/.  These shadow the
# current PYZ-compiled versions and cause ImportError when new symbols are added.
# PIL/ and pystray/ subdirectory .py files are intentional (collect_all) and must
# not be touched — glob('*.py') is non-recursive so they are safe.
if getattr(sys, "frozen", False):
    _meipass_root = Path(getattr(sys, "_MEIPASS", ""))
    for _stale_py in list(_meipass_root.glob("*.py")):
        try:
            _stale_py.unlink()
        except Exception:
            pass
...
def _ensure_dialog_above(
    dlg: "tk.Toplevel",
    parent: "tk.Tk | tk.Toplevel",
) -> None:
    """Force `dlg` above `parent` in Z-order using Win32 SetWindowPos.

    Tk's lift() alone is unreliable after an overrideredirect panel is
    restored from minimize: Windows places the panel at HWND_TOP during
    the overrideredirect re-application, and dialogs opened afterward may
    appear behind it — sometimes even below a non-topmost panel because
    Tk may accidentally set HWND_TOPMOST during the overrideredirect toggle.

    This function moves *parent*'s HWND to the position immediately after
    (below) *dlg*'s HWND in the Z-order — a robust, one-shot fix that works
    regardless of whether the panel is topmost or non-topmost.
    """
    ...
    try:
        ...
        dlg_h = int(user32.GetAncestor(ctypes.c_void_p(int(dlg.winfo_id())), GA_ROOT) or 0)
        par_h = int(user32.GetAncestor(ctypes.c_void_p(int(parent.winfo_id())), GA_ROOT) or 0)
        if dlg_h and par_h and dlg_h != par_h:
            # Insert parent AFTER (below) dialog in Z-order.
            # SWP_NOACTIVATE prevents stealing focus from the active dialog.
            user32.SetWindowPos(
                ctypes.c_void_p(par_h),
                ctypes.c_void_p(dlg_h),
                0, 0, 0, 0,
...
GUI_BTN_START_PNG       = _res.GUI_BTN_START_PNG
GUI_BTN_START_HOVER_PNG = _res.GUI_BTN_START_HOVER_PNG
GUI_BTN_STOP_PNG        = _res.GUI_BTN_STOP_PNG
...
def _build_dialog_caption(
    ...
    *,
    ...
    # Deferred Z-order fix (v1.0.77): after a minimize/restore cycle the
    # overrideredirect panel is placed at HWND_TOP by Windows during
    # overrideredirect re-application.  Dialogs that open afterward may
    # appear behind it.  Firing at 50 ms ensures this runs after the
    # caller's deiconify() + lift() and any pending after(0) callbacks.
    def _deferred_z_fix(d=dlg) -> None:
        ...
        try:
            _parent = d.master
            if _parent and _parent.winfo_exists():
                _ensure_dialog_above(d, _parent)
        except Exception:
            pass
    dlg.after(50, _deferred_z_fix)
...
def _show_plans_popup(parent, app) -> None:
    ...
    dialog.focus_force()
    dialog.grab_set()
    # Fix (v1.0.77): _show_plans_popup uses a standard Toplevel (no
    # _build_dialog_caption), so the deferred Z-order fix must be applied
    # here explicitly to handle the minimize/restore stacking bug.
    _ensure_dialog_above(dialog, parent)
...
class _ImageToggle(ttk.Checkbutton):
    ...
    Uses ttk.Checkbutton (native Windows checkbox) instead of custom PNG images.
    Keeps configure(text=, state=, cursor=), bind(), grid(), pack() compatibility.
    """
    ...
    def __init__(
        ...
        *,
        ...
        text: str = "",
        command=None,
        image_w: int = 42,   # ignored — kept for call-site compat
        image_h: int = 22,   # ignored
        ...
        kw: dict = {"variable": variable, "text": text, "command": _on_toggle}
    ...
    def configure(self, cnf=None, **kw):  # type: ignore[override]
        if cnf:
            kw.update(cnf)
        # Drop keys that ttk.Checkbutton does not accept
        kw.pop("cursor", None)
        super().configure(**kw)
...
I18N = {
    "en": {
        ...
        "help_hwid_body": (
            ...
            "  2. Select \"License\" from the menu\n"
            "  3. Your HWID is shown in the License window\n\n"
        ...
        "menu_help": "Help",
        "menu_help_about": "About WWM Overlay",
        "menu_help_license": "License",
        "menu_help_netoptimizer": "Network Optimizer",
        ...
        "trial_notice_title": "3-day trial started",
        "trial_notice_body":
            "Full features are unlocked for 3 days on this PC.\n\n"
            "Trial expires: {expires}\n\n"
            "After that, only the free overlay (FPS, CPU, GPU, VRAM) remains until you enter a license.",
        "menu_status_trial": "⏱ Trial: {time}",
        "menu_status_trial_expired": "⛔ Trial ended",
        "menu_status_licensed": "✓ License: {info}",
    ...
    "vi": {
        ...
        "menu_help": "Trợ giúp",
        "menu_help_about": "Thông tin WWM Overlay",
        "menu_help_license": "License",
        "menu_help_netoptimizer": "Tối ưu mạng",
        "settings_dialog_title": "WWM Overlay Settings",
        ...
        "trial_notice_title": "Đã bắt đầu dùng thử 3 ngày",
        "trial_notice_body":
            "Toàn bộ tính năng đã được mở khoá dùng thử trong 3 ngày trên PC này.\n\n"
            "Trial hết hạn: {expires}\n\n"
            "Sau thời gian này, app chỉ giữ lại Overlay miễn phí (FPS, CPU, GPU, VRAM) cho tới khi nhập License.",
        "menu_status_trial": "⏱ Trial: {time}",
        "menu_status_trial_expired": "⛔ Trial kết thúc",
        "menu_status_licensed": "✓ License: {info}",
...
def _start_trial_label_ticker(cp, root: tk.Tk) -> None:
    """Update the trial/license status label in the menu bar every 60 s.

    This runs independently of ``_start_access_refresh`` so the countdown
    ticks even when the licensed state hasn't changed.
    """
    def _tick() -> None:
        try:
            if cp is not None:
                cp._update_trial_label()
        except Exception:
            pass
        try:
            root.after(60_000, _tick)
...
class ControlPanel:
    def __init__(
        ...
        # ── System Menu bar ──────────────────────────────────────────────────
        self._menubar = tk.Menu(self.window, tearoff=False)
        self.window.configure(menu=self._menubar)

        # Account — opens User Center directly (no submenu)
        self._menubar.add_command(
            label="Account",
            command=lambda: (play_ui_click(), self._open_user_center()),
        )

        # Settings — opens settings dialog directly
        self._menubar.add_command(
            label="Settings",
            command=lambda: (play_ui_click(), self._open_settings_from_caption()),
        )

        # Help submenu — About, License, Network Optimizer
        self._menu_help = tk.Menu(self._menubar, tearoff=False)
        self._menubar.add_cascade(label="Help", menu=self._menu_help)
        self._menu_help.add_command(
            label="About WWM Overlay",
        ...
        self._menu_help.add_command(
            ...
            command=lambda: (play_ui_click(), self._open_optiz_nw()),
        )

        # Language toggle
        self._menubar.add_command(
            label="EN-VI",
            command=lambda: (play_ui_click(), self._toggle_caption_language()),
        )

        # Trial / License status label (index 4) — click opens license dialog
        self._menubar.add_command(
            label=self._trial_label_text(),
            command=lambda: (play_ui_click(), self._open_license_dialog()),
        )
        ...
        # ── Start / Stop image button (replaces old text button + logo) ──────
        # Three images: start (normal), start (hover), stop
        self._monitor_btn_start_photo = _load_ui_photo(GUI_BTN_START_PNG, 128, 76)
        self._monitor_btn_start_hover_photo = _load_ui_photo(GUI_BTN_START_HOVER_PNG, 128, 76)
        self._monitor_btn_stop_photo = _load_ui_photo(GUI_BTN_STOP_PNG, 128, 76)
        self._monitor_btn_label = tk.Label(
            self.monitoring_frame,
            bd=0,
            highlightthickness=0,
            cursor="hand2",
        )
        self._monitor_btn_label.grid(row=0, column=1, rowspan=3, sticky="e", padx=(12, 0))
        self._monitor_btn_label.bind("<Button-1>", lambda _e: self._on_monitor_toggle_clicked())
        self._monitor_btn_label.bind("<Enter>", self._on_monitor_btn_enter)
        self._monitor_btn_label.bind("<Leave>", self._on_monitor_btn_leave)
        self._monitor_btn_hovered: bool = False
    ...
    def _open_about_menu_from_caption(self) -> None:
        # Called from menubar "About" cascade — just open the about dialog directly.
        self._open_about_from_caption()

    def _open_optiz_nw(self) -> None:
        """Launch OptizNW.exe — the network optimizer companion app."""
        import subprocess as _sp
        import sys as _sys
        from pathlib import Path as _Path

        # When frozen: OptizNW.exe lives alongside PingOverlay.exe (install dir root)
        # When running from source: launch optiz_nw.py via the current Python interpreter
        if getattr(_sys, "frozen", False):
            exe_dir = _Path(_sys.executable).resolve().parent
            optiz_exe = exe_dir / "OptizNW.exe"
            if optiz_exe.exists():
                try:
                    _sp.Popen([str(optiz_exe)], creationflags=_sp.DETACHED_PROCESS)
                except Exception as exc:
                    import tkinter.messagebox as _mb
                    _mb.showerror(
                        "OptizNW",
                        f"Failed to launch OptizNW.exe:\n{exc}",
                        parent=self.window,
                    )
            else:
                import tkinter.messagebox as _mb
                _mb.showerror(
                    "OptizNW",
                    f"OptizNW.exe not found at:\n{optiz_exe}",
                    parent=self.window,
                )
        else:
            # Dev / source mode — run the module directly
            optiz_py = _Path(__file__).resolve().parent / "optiz_nw.py"
            try:
                _sp.Popen([_sys.executable, str(optiz_py)], creationflags=_sp.DETACHED_PROCESS)
            except Exception as exc:
                import tkinter.messagebox as _mb
                _mb.showerror(
                    "OptizNW",
                    f"Failed to launch optiz_nw.py:\n{exc}",
                    parent=self.window,
                )
    ...
    def _open_help_popup(self) -> None:
        ...
        apply_custom_cursor(dlg)
        bind_ui_click_sound(dlg)

        # ── content ────────────────────────────────────────────────────────
        frame = ttk.Frame(dlg, padding=(20, 16))
        ...
        ttk.Button(frame, text="OK", command=dlg.destroy).pack(pady=(16, 0))
    ...
    def _open_user_center(self) -> None:
        ...
        dialog.title(tr(self.app, "uc_title"))
        ...
        outer = ttk.Frame(dialog, padding=(16, 12, 16, 16))
        outer.pack(fill="both", expand=True)
        ...
        def _render(sess: dict | None) -> None:
            ...
            if not sess:
                ...
                def _do_logout() -> None:
                    ...
                    def _bg() -> None:
                        _acc.logout(token)
                        # Clear local license file and all server-license state
                        # immediately so the menu bar doesn't keep showing
                        # "License: Active" until the next heartbeat.
                        try:
                            import license_check as _lc_lo
                            _lc_lo.clear_license()
                            # Reset server-side license flags immediately
                            self.app.license_server_licensed = False
                            self.app.license_server_expires_at = ""
                            self.app.license_server_plan_id = ""
                            _apply_license_status(self.app, _lc_lo.current_status(), create_trial=True)
                        except Exception as _exc:
                            print(f"[uc] logout license clear: {_exc}")
                        try:
                            dialog.after(0, lambda: (_render(None), _center_dialog()))
                        except Exception:
                            pass
                        try:
                            dialog.after(0, lambda: refresh_control_panel(self.app))
                        except Exception:
                            pass
    ...
    # ── Trial / License menu bar label ───────────────────────────────────────

    def _trial_label_text(self) -> str:
        """Return the text for the trial/license status menu item (index 4).

        Priority: offline HMAC key → Supabase server license → remote trial
        (heartbeat backend) → local trial (trial.json) → expired.

        All countdowns are computed in real-time from absolute ISO timestamps
        so they stay accurate between heartbeat / access-refresh ticks.
        """
        ...
        def _remaining(iso: str) -> int | None:
            """Seconds until `iso` timestamp, computed right now. None on error."""
            if not iso:
                return None
            try:
                exp = _dt.fromisoformat(str(iso).replace("Z", "+00:00"))
                if exp.tzinfo is None:
                    exp = exp.replace(tzinfo=_tz.utc)
                return max(0, int((exp - _dt.now(_tz.utc)).total_seconds()))
            except Exception:
                return None

        def _exp_label(iso: str) -> str:
            """Human-readable expiry from ISO string (days remaining)."""
            secs = _remaining(iso)
            if secs is None:
        ...
        # ── 1. Offline HMAC key ───────────────────────────────────────────
        if bool(getattr(self.app, "license_key_valid", False)):
            status = getattr(self.app, "license_status", None)
            exp = getattr(status, "expires_at", None)
            if exp == "lifetime":
                info = "Lifetime"
            elif exp:
                info = _exp_label(str(exp))
            else:
                info = "Active"
            return tr(self.app, "menu_status_licensed", info=info)

        # ── 2. Supabase server license ────────────────────────────────────
        if bool(getattr(self.app, "license_server_licensed", False)):
            srv_exp = str(getattr(self.app, "license_server_expires_at", "") or "")
            info = _exp_label(srv_exp) if srv_exp else "Active"
            return tr(self.app, "menu_status_licensed", info=info)

        # ── 3. Remote trial (heartbeat backend — authoritative) ───────────
        if bool(getattr(self.app, "remote_trial_active", False)):
            exp_str = str(getattr(self.app, "remote_trial_expires_at", "") or "")
            secs = _remaining(exp_str)
            if secs is None:
                # Fall back to last-known stale value from heartbeat
                secs = max(0, int(getattr(self.app, "remote_trial_seconds_remaining", 0)))
            if secs == 0:
                return tr(self.app, "menu_status_trial_expired")
            return tr(self.app, "menu_status_trial", time=_fmt_secs(secs))

        # ── 4. Local trial (trial.json — used when heartbeat disabled) ────
        trial = getattr(self.app, "trial_status", None)
        if trial and bool(getattr(trial, "has_started", False)):
            exp_str = str(getattr(trial, "expires_at", "") or "")
            secs = _remaining(exp_str)
            if secs is None:
                secs = max(0, int(getattr(trial, "seconds_remaining", 0)))
            if secs > 0:
                return tr(self.app, "menu_status_trial", time=_fmt_secs(secs))
            return tr(self.app, "menu_status_trial_expired")

        # ── 5. Expired / not started ──────────────────────────────────────
        return tr(self.app, "menu_status_trial_expired")

    def _update_trial_label(self) -> None:
        """Refresh only the trial/license status entry in the menu bar."""
        try:
            self._menubar.entryconfig(4, label=self._trial_label_text())
        except Exception:
            pass
        ...
        try:
            if self.window.state() != "iconic":
                ...
                if self._first_shown:
                    self.window.after(10, self._drop_topmost)
        except Exception:
            pass
    ...
    def refresh(self) -> None:
        ...
        free_options = {"show_fps", "show_cpu_temp", "show_gpu_temp", "show_vram"}
        ...
        # Update menubar item labels to match the current language.
        # Menubar indices: 0=Account, 1=Settings, 2=Help(cascade), 3=EN-VI(fixed), 4=trial/license
        # Help submenu indices: 0=About, 1=License, 2=<separator>, 3=Network Optimizer
        try:
            self._menubar.entryconfig(0, label=tr(self.app, "menu_account"))
            self._menubar.entryconfig(1, label=tr(self.app, "menu_settings"))
            self._menubar.entryconfig(2, label=tr(self.app, "menu_help"))
            # index 3 "EN-VI" is always fixed — no entryconfig needed
            self._menu_help.entryconfig(0, label=tr(self.app, "menu_help_about"))
            self._menu_help.entryconfig(1, label=tr(self.app, "menu_help_license"))
            # index 2 is separator — skip
            self._menu_help.entryconfig(3, label=tr(self.app, "menu_help_netoptimizer"))
        except Exception:
            pass
    ...
    # ── Start/Stop image button helpers ─────────────────────────────────────

    def _sync_monitor_btn_image(self) -> None:
        """Update the monitor image button to reflect current monitoring state."""
        if self.app.monitoring_enabled:
            photo = self._monitor_btn_stop_photo
        elif self._monitor_btn_hovered:
            photo = self._monitor_btn_start_hover_photo
        else:
            photo = self._monitor_btn_start_photo
        if photo is not None:
            self._monitor_btn_label.configure(image=photo, text="")
            self._monitor_btn_label.image = photo
        else:
            # Text fallback if images not found
            self._monitor_btn_label.configure(
                image="",
                text="■ Stop" if self.app.monitoring_enabled else "▶ Start",
            )

    def _on_monitor_btn_enter(self, _event=None) -> None:
        self._monitor_btn_hovered = True
        if not self.app.monitoring_enabled:
            self._sync_monitor_btn_image()

    def _on_monitor_btn_leave(self, _event=None) -> None:
        self._monitor_btn_hovered = False
        if not self.app.monitoring_enabled:
            self._sync_monitor_btn_image()
    ...
    def _open_hotkey_settings(self) -> None:
        ...
        dialog = tk.Toplevel(self.window)
        dialog.title(tr(self.app, "settings_dialog_title"))
        dialog.transient(self.window)
        dialog.resizable(False, False)
        _apply_window_icon(dialog)
        apply_custom_cursor(dialog)
    ...
    def _open_license_dialog(self) -> None:
        dialog = tk.Toplevel(self.window)
        dialog.title(tr(self.app, "settings_license_section"))
        dialog.transient(self.window)
        dialog.resizable(False, False)
        _apply_window_icon(dialog)
        apply_custom_cursor(dialog)
...
def main():
    ...
    # Merge bundled internal_config.json on top of the user config.
    # System-controlled keys (license_admin, release_gateway, update routing)
    # live only here — they must never be stored in the user's config.json so
    # they stay current after every installer/incremental build without the user
    # having to reinstall or clear their config.
    try:
        _icfg_path = _res.BUNDLED_INTERNAL_CONFIG
        if _icfg_path.exists():
            import json as _json_ic
            with open(_icfg_path, "r", encoding="utf-8") as _f:
                _icfg = _json_ic.load(_f)
            if isinstance(_icfg, dict):
                # Top-level system-only keys: always override user config
                for _k in ("license_admin", "release_gateway", "github_token",
                           "bridge_to_installer"):
                    if _k in _icfg:
                        cfg[_k] = _icfg[_k]
                # Update sub-keys: override routing but preserve user flags
                if "update" in _icfg:
                    _upd = cfg.setdefault("update", {})
                    for _k in ("repo", "tag_prefix", "asset_prefix",
                               "fallback_asset_prefixes", "asset_extension",
                               "metadata_timeout_seconds",
                               "startup_probe_wait_seconds"):
                        if _k in _icfg["update"]:
                            _upd[_k] = _icfg["update"][_k]
            print(f"[main] internal_config merged from {_icfg_path}")
        del _icfg_path
    except Exception as _exc:
        print(f"[main] internal_config merge skipped: {_exc}")
    ...
    _start_trial_label_ticker(app.control_panel, overlay.root)
