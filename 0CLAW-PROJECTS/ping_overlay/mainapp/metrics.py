...
def _query_gpu_temperature_c(timeout_s: float = 4.0) -> float | None:
    """Best-effort GPU temperature trên Windows.
    Ưu tiên:
      1. LibreHardwareMonitor / OpenHardwareMonitor (CIM, vendor-agnostic)
      2. nvidia-smi (NVIDIA, không cần LHM)
    Trả None nếu không tìm được sensor phù hợp.
    """
    script = r"""
$ErrorActionPreference = 'SilentlyContinue'
$namespaces = @('root/LibreHardwareMonitor', 'root/OpenHardwareMonitor')
foreach ($ns in $namespaces) {
  $sensor = Get-CimInstance -Namespace $ns -ClassName Sensor -ErrorAction SilentlyContinue |
    Where-Object {
      $_.SensorType -eq 'Temperature' -and (
        $_.Name -match 'GPU' -or $_.Name -match 'Video Card'
      )
    } |
    Sort-Object { if ($_.Name -match 'GPU Core') { 0 } else { 1 } } |
    Select-Object -First 1 -ExpandProperty Value
  if ($null -ne $sensor -and "$sensor" -ne '') {
    [Console]::WriteLine([double]$sensor)
    exit 0
...
# Fallback: nvidia-smi (NVIDIA only)
$nvOut = & nvidia-smi --query-gpu=temperature.gpu --format=csv,noheader,nounits 2>$null
if ($LASTEXITCODE -eq 0 -and $null -ne $nvOut) {
  $line = ($nvOut -split "`n")[0].Trim()
  if ($line -match '^\d+(\.\d+)?$') {
...
class MemoryMonitor:
    ...
    def __init__(self, pid: int, poll_sec: float = 1.5):
        ...
        self._gpu_temp_last_call: float = 0.0
        self._gpu_temp_value: float | None = None
        self._gpu_temp_interval_s: float = 15.0
    ...
    def _loop(self) -> None:
        ...
        while not self._stop:
            ...
            if now - self._gpu_temp_last_call >= self._gpu_temp_interval_s:
                self._gpu_temp_last_call = now
                gtemp = _query_gpu_temperature_c()
                if gtemp is not None:
                    self._gpu_temp_value = gtemp
            ...
            with self._lock:
                ...
                self._snap["gpu_temp_c"] = self._gpu_temp_value
